	...
	if($_POST["accion"]=="insertar"){
        if (!empty($_POST["nombre"]) && !empty($_POST["descripcion"]) && !empty($_POST["precio"]) && !empty($_POST["usuario"]) && !empty($_POST["etiqueta"])) {
          ...
          move_uploaded_file($tmp_name, $ruta);
           $insercion=$libro->insertarLibro($_POST["nombre"],$_POST["descripcion"],$_POST["precio"],$_POST["etiqueta"],$_POST["usuario"],$ruta);
                if (!$insercion) {
                ...
                //recuperar de la bbdd
                  ...
                  // echo "<a href='actualizarLibro.php?titulo=".$value["titulo"]."&subtitulo=".$value["subtitulo"]."&descripcion=".$value["descripcion"]."&categoria=".$value["categoria"]." '>Actualizar Articulo</a><br>";

                  // echo "<a href='borrarLibro.php?titulo=".$value["titulo"]."'>Borrar Articulo</a><br>";
                  }
        }else{
...
<section>
    <article class="parte2">
      <form class="" action="insertarLibro.php" method="post" enctype="multipart/form-data">
